...
class Posmegamenu extends Module {
  ...
  private $page_name = '';
    ...
    public function install() {
        ...
        Configuration::updateValue($this->name . '_menu_depth', 1);
        Configuration::updateValue($this->name . '_merge_cate', 1);
        Configuration::updateValue($this->name . '_show_depth', 1);
    ...
    private function getCategoryOptionLink($id_category = 1, $id_lang = false, $id_shop = false, $recursive = true)
  {
    ...
    public function drawCustomMenuBlock($blockId, $bc) {
        ...
        $blockHtml = str_replace('/pos_rossi/',__PS_BASE_URI__,$bc['description']);
        $drawPopup = $blockHtml;
        if ($drawPopup) {
      ...
      switch (substr($item, 0, strlen($value[1])))
      {
        ...
        case 'LNK':
          ...
          if (count($link))
          {
            ...
            $this->_menuLink .= '<div class ="pt_menu pt_menu_link"><div class="parentMenu" ><a href="/'.Tools::HtmlEntitiesUTF8($link[0]['link']).'"'.(($link[0]['new_window']) ? ' target="_blank"': '').'><span>'.$link[0]['label'].'</span></a></div></div>'.PHP_EOL;
          }
          break;
