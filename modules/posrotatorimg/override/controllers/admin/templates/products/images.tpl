	...
	<table class="table tableDnD" id="imageTable">
		<thead>
			<tr class="nodrag nodrop"> 
				...
				<th class="fixed-width-xs"><span class="title_box">{l s='Rotator'}</span></th>
				<th></th> <!-- action -->
			</tr>
		</thead>
		<tbody id="imageList">
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$.ajax({
					...
					data: 'id_product='+id_product+'&action=2',
					dataType: 'json',
					success: function(json) {
						var td = '#td_'+json;
						var rotator = $(td).next().next().find('input').attr('checked','checked');
					}
				});
			...
			$('input[name="rotator"]').live('click',function(){
				...
				$.ajax({
						type: 'POST',
						url:'../modules/posrotatorimg/ajax.php',
						data: 'img_id='+id+'&id_product='+id_product+'&action=1',
						dataType: 'json',
						success: function(json) {
