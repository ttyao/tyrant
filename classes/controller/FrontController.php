...
class FrontControllerCore extends Controller
{
	...
	protected function canonicalRedirection($canonical_url = '')
	{
		...
		if (!preg_match('/^'.Tools::pRegexp(rawurldecode($canonical_url), '/').'([&?].*)?$/', $match_url))
		{
			...
			if (defined('_PS_MODE_DEV_') && _PS_MODE_DEV_ && $_SERVER['REQUEST_URI'] != __PS_BASE_URI__ && strpos($final_url, 'order_confirm') != false) {
	...
	public function setMobileMedia()
	{
		...
		if (!file_exists($this->getThemeDir().'css/autoload/'))
		{
			...
			$this->addCSS(_THEME_MOBILE_CSS_DIR_.'global_ext.css', 'all');
		}
	}
	...
	public function setMedia()
	{
		...
		$this->addCSS(_THEME_CSS_DIR_.'global_ext.css', 'all');
		$this->addjquery();
